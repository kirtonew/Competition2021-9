...
def trainModel(diseaseType, datas, datasLabel):
    ...
    #  存储模型数据
    data_dealing.saveModel(diseaseType, 'AdaBoostingModel', model)
    data_dealing.saveAccuracy(diseaseType, 'AdaBoostingModel', accuracy)
...
def predict(diseaseType, refinedData, datas, datasLabel):
    #  获得模型及其准确率
    model = data_dealing.getModel(diseaseType, 'AdaBoostingModel')
    accuracy = data_dealing.getAccuracy(diseaseType, 'AdaBoostingModel')
...
if __name__ == '__main__':
    ...
    #  输出结果
    print("病人{}的概率是".format(disease[predictResult]) + str(round(accuracy * 100, 3)) + '%')
