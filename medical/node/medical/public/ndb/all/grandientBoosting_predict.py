...
def trainModel(diseaseType, datas, datasLabel):
    ...
    #  存储模型数据
    data_dealing.saveModel(diseaseType, 'GradientBoostingModel', model)
    data_dealing.saveAccuracy(diseaseType, 'GradientBoostingModel', accuracy)
...
def predict(diseaseType, refinedData, datas, datasLabel):
    #  获得模型及其准确率
    model = data_dealing.getModel(diseaseType, 'GradientBoostingModel')
    accuracy = data_dealing.getAccuracy(diseaseType, 'GradientBoostingModel')
