...
app.all('*', function(req, res, next) {
  res.header("Access-Control-Allow-Origin", "http://39.106.209.229:8080");
...
io.on('connection', socket =>{
  socket.on("login_user",data => {
    ...
    let doctor = userList.length!==0 ? userList.find(item => item.doctorIndex === data.doctorIndex):false
    ...
    if(user){
      ...
      socket.emit("userExit",{msg:"医生未上班，请等候"})
    }
  })
  socket.on("login_doctor",data => {
    // 判断，如果data在userList数组中存在，说明该用户已经登录，不允许登录
    // 如果data在userList数组中不存在，说明该用户没有登录，允许登录
    let user =userList.length!==0 ? userList.find(item => item.username === data.username):false
